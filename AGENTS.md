...
This file provides guidance to Gemini (Antigravity) and other agents when working with code in this repository.
...
- Allowed commit types: `feat`, `fix`, `refactor`, `chore`, `docs`, `test`, `style`, `perf`
## Conventions
...
- Test method names should be written in Korean using backticks (e.g., `` `고정 지출을 생성할 수 있다`() ``)
- Testing framework uses JUnit 5 and Mockito-Kotlin (`whenever`, `verify`, etc.)
